...
def eval_model(args):
    ...
    for row in tqdm(questions, total=len(questions)):
        ...
        image = image = Image.open(os.path.join(args.image_folder, row['image'])).convert("RGB")
        ...
        print(outputs)
        answer.append([idx, {'text': outputs}, question_type])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", type=str, default="/model_cache/MGM-34B-HD")
    ...
    parser.add_argument("--answers-file", type=str, default="result/mgm_result.json")
    ...
    parser.add_argument("--single-pred-prompt", default=True)
